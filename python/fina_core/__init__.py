...
extension (``fina_core._core``) while the Python layer exposes a clean, small API.
...
from .integrations import register_fina_handlers
...
__all__ = [
    ...
    "ETL_SCHEMA",
    "expand_etl_config",
] + ["Scheduler", "TaskHook", "ScheduledRun", "run_pipelines_scheduled", "Event", "EventBus", "ProcessInstance", "SchedulerService", "ThreadInstance", "render_parameters", "register_fina_handlers"]
...
class PipelinesConfig:
    ...
    Example::
        ...
        result = fina_core.run_pipelines(cfg)
    ...
    run (see ``docs/scheduler.md`` and ``run_pipelines_scheduled``)::
        ...
        result = fina_core.run_pipelines_scheduled(cfg.to_yaml(), workers=10)
    """
...
def run_pipelines(
    ...
    *,
    ...
    Examples
    --------
    >>> r = fina_core.run_pipelines("examples/demo/pipelines.yml")
    >>> r.rows
    {'spot': 12, 'products': 36}
    """
...
def main() -> None:  # pragma: no cover - convenience CLI
    if len(sys.argv) < 2:
        print("usage: python -m fina_core <pipelines.yml>")
        return
    config = sys.argv[1]
